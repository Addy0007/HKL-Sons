    ...
    >
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-[#2C2C2C] mb-1">Reset Password</h2>
          <p className="text-sm text-[#3D3D3D]">Enter your new password below.</p>
        </div>

        <div className="bg-[#F6F3EC] shadow-xl rounded-2xl p-8">
          {message && (
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative">
                <Lock className="absolute left-3 top-2.5 h-5 w-5 text-[#C6A15B]" />
                <input
                  type={show ? "text" : "password"}
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  className="w-full pl-10 pr-10 py-2.5 border border-[#C6A15B]/30 rounded-lg bg-[#F6F3EC] text-[#2C2C2C] focus:ring-2 focus:ring-[#1F3D2B] outline-none"
                  placeholder="••••••••"
                />
                <button
                  type="button"
                  onClick={() => setShow(!show)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 p-1 rounded hover:bg-[#EDE9E0] text-[#C6A15B]"
                >
